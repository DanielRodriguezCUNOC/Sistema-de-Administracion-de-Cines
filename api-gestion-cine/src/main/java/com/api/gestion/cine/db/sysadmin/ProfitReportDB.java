...
public class ProfitReportDB {
  ...
  public List<CinemaCostReport> getCinemaCosts(LocalDate fechaInicio, LocalDate fechaFin) throws Exception {
    List<CinemaCostReport> cinemaCosts = new ArrayList<>();
    Connection conn = DBConnectionSingleton.getInstance().getConnection();

    // 🔹 Construimos SQL dinámicamente según las fechas
    StringBuilder sql = new StringBuilder(
        "SELECT c.id_cine, c.nombre_cine, " +
    ...
    if (fechaInicio != null && fechaFin != null) {
      sql.append("WHERE cc.fecha_modificacion BETWEEN ? AND ? ");
    }

    sql.append("GROUP BY c.id_cine, c.nombre_cine ORDER BY c.id_cine");
    ...
    try (PreparedStatement pstmt = conn.prepareStatement(sql.toString())) {
      ...
      try (ResultSet rs = pstmt.executeQuery()) {
        while (rs.next()) {
          ...
          if (fechasStr != null) {
            for (String f : fechasStr.split(",")) {
              fechas.add(LocalDate.parse(f.trim()));
            }
          }
          ...
          if (costosStr != null) {
            for (String c : costosStr.split(",")) {
              costosList.add(new BigDecimal(c.trim()));
            }
          }

          CinemaCostReport report = new CinemaCostReport(idCine, nombreCine, fechas,
              costosList);
  ...
  public List<AdvertisementProfitReport> getAdvertisementPayments(LocalDate fechaInicio, LocalDate fechaFin)
    ...
    StringBuilder sql = new StringBuilder(
        "SELECT pa.id_pago_anuncio AS id, a.nombre_anuncio AS nombre, " +
            "pa.fecha_pago, pa.monto_pago AS montoPago " +
            "FROM pago_anuncio pa " +
            "INNER JOIN anuncio a ON pa.id_anuncio = a.id_anuncio ");

    // 🔹 Solo aplicar filtro de fecha si existen ambas fechas
    if (fechaInicio != null && fechaFin != null) {
      sql.append("WHERE pa.fecha_pago BETWEEN ? AND ? ");
    }

    sql.append("ORDER BY pa.fecha_pago");
  ...
  public List<AdvertisementProfitReport> getAdBlockPayments(LocalDate fechaInicio, LocalDate fechaFin)
    ...
    StringBuilder sql = new StringBuilder(
        "SELECT poa.id_pago_ocultacion AS id, u.nombre_completo AS nombre, " +
            "poa.fecha_pago, poa.monto AS amount " +
            "FROM pago_ocultacion_anuncios poa " +
            "INNER JOIN usuario u ON poa.id_usuario = u.id_usuario ");

    // 🔹 Agregamos WHERE solo si se especifican fechas
    if (fechaInicio != null && fechaFin != null) {
      sql.append("WHERE poa.fecha_pago BETWEEN ? AND ? ");
    }

    sql.append("ORDER BY poa.fecha_pago");
